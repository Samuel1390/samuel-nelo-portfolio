...
const MY_EMAIL = "snelo1390@gmail.com";
...
const ContactForm = () => {
  ...
  async function handleClickSubmit(e) {
    e.preventDefault();
    if (!EMAIL_REGEX.test(emailRef.current.value) || message.length < 1) {
      Swal.fire({
        icon: "error",
        title:
          language === "spanish"
            ? "Error al enviar el mensaje, el email no es válido"
            : "Error to send the menssage, the email is not valid",
      });
      return;
    }
    setIsLoading(true);
    ...
    if (result?.status >= 400) {
      Swal.fire({
        ...
        text:
          language === "spanish"
            ? "Si deseas enviarme un email: " + MY_EMAIL
            : "If you wish send me an  email: " + MY_EMAIL,
      });
      setIsLoading(false);
      return;
    }
